...
\begin{align}
	...
	\implies &\frac{d_x}{d_z} = - \frac{n_z}{n_x} - \frac{n_yd_y}{n_xd_z}
\end{align}
We can substitute this into what we had in part 1, i.e.
\begin{align}
	x_v &= \frac{fd_x + p_xd_x}{d_z} = f \frac{d_x}{d_z} + p_x \\
	&= f \left(- \frac{n_z}{n_x} - \frac{n_yd_y}{n_xd_z} \right) + p_x \\
	&= -f \frac{n_z}{n_x	} - f \frac{n_yd_y}{n_xd_z} + p_x = (\dag)
\end{align}
also, 
\begin{align}
...
\begin{align}
	y_v 
	= (\dag) 
	&= -f \frac{n_z}{n_x} - f\frac{n_y (y_v - p_y)}{n_x f} + p_x
\end{align}
from where we can simplify
\begin{align}
	&-fn_z - n_y(y_v - p_y) + p_xn_x = n_xx_v \\
	\implies &-fn_z - n_yy_v + n_yp_y + p_xn_x = n_xx_v \\
	\implies &\underbrace{-n_x }_{\alpha}x_v \underbrace{- n_y}_{\beta}y_v = \underbrace{fn_z - n_yp_y - n_xp_x}_{\text{constant independent of $d$}}
\end{align}
...
\begin{equation}
	...
	\end{bmatrix}
\end{equation}
where $a, ax, ay$ and all the $p_{(\cdot\cdot)}$ are known while $X, Y, Z$ are unknowns. We can obtain the following solvable linear relationship
\begin{equation}
