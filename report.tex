...
\begin{align}
	y_v &= \frac{fd_y + p_yd_z}{d_z} = f \frac{d_y}{d_z} + p_y \\
	&= f \left(- \frac{n_z}{n_x} - \frac{n_yd_y}{n_zd_z} \right) + p_y \\
	&= -f \frac{n_z}{n_x	} - f \frac{n_yd_y}{n_zd_z} + p_y = (\dag)
\end{align}
also, 
\begin{align}
	x_v = \frac{fd_x + p_xd_z}{d_z} = f \frac{d_x}{d_z} + p_x \implies \frac{d_x}{d_z} = \frac{x_v - p_x}{f}
\end{align}
Then, 
\begin{align}
	&y_v = (\dag) = -f \frac{n_z}{n_x} - f\frac{n_y (x_v - p_x)}{n_z f} + p_y \\
	\implies &-f^2n_z^2 - fn_zn_y(x_v - p_x) + n_xn_zfp_y = n_xn_zfy_v \\
	\implies &\underbrace{- fn_zn_y}_{\alpha}x_v + \underbrace{-f^2n_z^2 + fn_zn_yp_x + n_xn_zfp_y}_{\text{constant independent of $d$}} = \underbrace{n_xn_zf}_{\beta}y_v
\end{align}
